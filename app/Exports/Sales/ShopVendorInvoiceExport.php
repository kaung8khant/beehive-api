...
class ShopVendorInvoiceExport implements WithColumnFormatting, WithColumnWidths, WithDrawings, WithHeadings, WithStyles, WithTitle
{
    protected $param;
    protected $from;
    protected $to;
    ...
    public function headings(): array
    {
        ...
        $shopOrderVendors = ShopOrderVendor::where('shop_id', $shop->id)
        ...
        foreach ($shopOrderVendors as $vendor) {
            $commissionCt = $vendor->commission * 0.05;

            $commissionSum +=  $vendor->commission;
        ...
        return [
            ...
            [1, 'Vendor Revenue (inc CT)', number_format($shopOrderVendors->count()), $shopOrderVendors->sum('total_amount')],
            [2, "Invoice for {$month} commission", '', $commissionSum ? $commissionSum : '0'],
            [3, 'CT on commission', '', $commissionCtSum ? $commissionCtSum : '0'],
            ['', 'Payable to Vendor', '', $shopOrderVendors->sum('total_amount') - $commissionSum - $commissionCtSum],
            [],
            [],
            ['Vendor Bank Info:'],
            ['Beneficiary:'],
